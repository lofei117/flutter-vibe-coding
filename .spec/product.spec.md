...
- Human in the loop 是远期能力：未来可在 App 端确认计划、diff 或风险后再应用变更，MVP 只保留接口和规划，不重投入。
...
- MVP 阶段不实现完整 Human in the loop 审批流，只保留远期接口设计。
...
MVP 不实现完整审批，但数据契约中应预留 `requiresApproval`、`approvalRequest`、`approvalDecision` 等概念。
